...
import static com.ericsson.component.aia.sdk.applicationmanager.services.configuration.TestConstants.TEST_TEMPLATE_PBA_JSON;
...
public class CreateApplicationEndPointIntegrationTest {
    ...
    @Test
    public void shouldReturnOkStatusWhenCreateApplicationIsSuccessful() throws Exception {
        final String templatePbaId = testUtils.createTemplate(TEST_TEMPLATE_PBA_JSON);
        ...
        pbaAsString = JsonPath.using(configuration).parse(pbaAsString).set("$.pba.templateInfo.id", templatePbaId).json().toString();

        final HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        final HttpEntity<String> entity = new HttpEntity<>(pbaAsString, headers);

        final ResponseEntity<String> response = this.restTemplate.exchange("/applications", HttpMethod.POST, entity, String.class);
        assertThat(response.getStatusCodeValue(), equalTo(200));
    }

    @Test
    public void shouldReturnErrorWhenTemplateDoesntExist() throws Exception {
        final File pbaJson = new File(TEST_PBA_JSON_ACTIVE);
